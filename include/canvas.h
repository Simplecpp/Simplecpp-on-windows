...
namespace canvas{
  ...
  void drawLine(XPoint start, XPoint end, Color line_color, unsigned int line_width=2);

  void drawPoint(XPoint point, Color point_color);

  void drawCircle(XPoint centre, int radius, Color fill_color, bool fill=true, unsigned int line_width=2, int line_style=SOLID_LINE); 

  void drawEllipse(XPoint centre, int width, int height, Color fill_color, bool fill=true, unsigned int line_width=2, int line_style=SOLID_LINE); 

  void drawPolygon(XPoint *points, int npoints, Color fill_color, bool fill=true, unsigned int line_width=2, int line_style=SOLID_LINE); 

  void drawText(XPoint position, const char *text, Color clr);
  void drawText(Position position, string text, Color clr = COLOR(0,0,255)); 
  void drawText(float x, float y, string text, Color clr = COLOR(0,0,255));
...
char charFromEvent(XEvent &event);  // return char whose key was pressed.
 bool checkEvent(XEvent &event);   // true if some event happened since last nextEvent.
